...
export class FirebaseProvider {
  ...
  timeline(timelineId, data) {
    ...
    this.dataProvider.getUser(loggedInUserId).take(1).subscribe((account) => {
      ...
      this.angularDb.object('/accounts/' + loggedInUserId + '/baseData/timeline/' + timelineId).update(timelineTemp);
  ...
  commentPost(postId, comment) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.angularDb.list("/comments/" + postId)
      .push(comment)
      .then((success) => {
        resolve(true);
      }).catch((error) => {
  ...
  commentReply(postId, key, comment) {

    return new Promise((resolve, reject) => {

      this.angularDb.list("/comments/" + postId + "/" + key + "/reply/")
      .push(comment)
      .then((success) => {
        resolve(true);
      }).catch((error) => {
  ...
  removeComment(postKey, key, subKey) {

    console.log("removeComment 2 ==> postKey("+postKey+") key("+key+") subKey("+subKey+")");

    if(subKey == 0) {
      this.angularDb.object('comments/' + postKey + "/" + key).remove();
    } else {
      this.angularDb.object('comments/' + postKey + "/" + key + "/reply/" + subKey).remove();
    }
  }
}
